...
def read_file(path):
    ...
    if content.shape[1] == 7:
        content.columns = ['timestamp', 'Volt', 'PAR',
                           'CO2', 'temp', 'logger', 'H2O']
    elif content.shape[1] == 6:
        content.columns = ['timestamp', 'CO2', 'PAR',
                           'temp', 'logger', 'H2O']
    else:
...
#   't' if transparent,
#   3.quality: 1 if good meaurment 2 if critical,
#   (highly faulty classification)
...
def find_aoi(content, tolerance=5):
    ...
    for measurment in range(no_measurments):
        ...
        # determines the start and end of
        # the logger range, including some tolerance
        ...
        for i in range(start_of_logger_area, end_of_logger_area):
            p, cov = np.polyfit(np.arange(aoi_length),
                                content.CO2.values[i:i+aoi_length],
                                1, cov=True)
        ...
        if best_score != 1:
            ...
            sec_index_end = best_index+aoi_length+1
            sec_index[best_index:sec_index_end] = 5*(np.arange(aoi_length+1))
...
def format_file(path_to_source, path_to_destination=None):
    ...
    print('Finding areas of intrest.')
        ...
        for file in os.listdir(args.source):
            ...
            if (os.path.isfile(path_to_file) and file[-4:] == '.txt'):
                if args.destination is None:
                    ...
                    path_to_destination = os.path.join(args.destination,
                                                       file[:-4]+'.xlsx')
